...
wchar_t* JieXiXML(wstring wxml)
{
	_bstr_t t = wxml.c_str();
	char* pchar = (char*)t;
	string xml = pchar;
	...
	XMLElement* titleElement = doc.FirstChildElement("msg")->FirstChildElement("appmsg")->FirstChildElement("mmreader")->FirstChildElement("template_detail")->FirstChildElement("line_content")->FirstChildElement("lines")->FirstChildElement("line")->FirstChildElement("value")->FirstChildElement("word");
	
	
	 const char *CStr = titleElement->GetText();
...
void SendWxMessage()
{
	...
	if (StrStrW(msg->wxid, L"gh"))
	{
		...
		{
			//wchar_t a1[200];
			//wcscpy_s(a1, JieXiXML(fullmessgaedata));
			//SendTextMessage((wchar_t*)L"wxid_fhjzylwfsxre22",a1 );
			swprintf_s(msg->content, L"%s", fullmessgaedata.c_str());
			msg->isMoney = TRUE;
		}
		else
		{
